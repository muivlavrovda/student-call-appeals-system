...
миграции базы данных, а затем поднимает сервер разработки. Управляющие команды
по-прежнему доступны через ``manage.py`` (``python manage.py <команда>``); этот
файл нужен лишь для простого старта одной командой ``python main.py``.
...
def main() -> None:
    ...
    # Сначала приводим схему БД в актуальное состояние, затем запускаем сервер.
